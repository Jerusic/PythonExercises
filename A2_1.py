...
def genSine(A, f, phi, fs, t):
    ...
    """
    ## Your code here
    t_range=np.arange(0,t,1.0/fs)
    x=A*np.cos(2*np.pi*f*t_range+phi)
    if fs<=2*f or A<=0 or t<=0 or fs<=0:
       print("fs, sampling frequency, must be greater than at least twice f, frequency of the sinusoid in Hz.  Also A, t, and fs must be positive")
    else:
        print(x)
        return(x)
